...
class Utilities
{
  public:
    ...
    static int GetFirstLineLength(const std::filesystem::path &Path);
...
std::vector<std::string> Utilities::ReadLines(const std::filesystem::path &Path)
{
    ...
    while (std::getline(FileStream, CurrentLine)) { AllLines.push_back(CurrentLine); }
...
std::vector<std::bitset<bits>> Utilities::ConvertToBitset(const std::vector<std::string> &Strings)
{
    ...
    for (const std::string &StringBytes : Strings) { Bitsets.emplace_back(StringBytes); }
    return Bitsets;
}
...
std::vector<int> Utilities::ConvertToInt(const std::vector<std::string> &Vec)
{
    ...
    for (const std::string &Str : Vec) { ReturnVec.push_back(std::stoi(Str)); }
...
std::vector<int> Utilities::GetBoardRow(const std::string& BasicString)
{
    ...
    for (int i = 1; i <= slots; i++)
    {
        ...
        if (i % 3 == 1) ReturnVec.push_back(stoi(Number));
    }

    return ReturnVec;
}
int Utilities::GetFirstLineLength(const std::filesystem::path &Path)
{
    VerifyElseCrash(std::filesystem::exists(Path));
